...
def internet_archive_fetch_resources(token, query_parameter):
    """
...
def internet_archive_fetch_resource(token, resource_id):
    """
    Fetch the Internet Archive resource matching the resource_id given.

    Parameters
    ----------
    token : str
        User's Internet Archive token
    ...
    Returns
    -------
    A dictionary object that represents the Internet Archive resource.
    Dictionary must be in the following format:
    {
        "kind": "container",
        "kind_name": "project",
    ...
    #     project_url = "https://archive.org/metadata/{}".format(project_id)
    ...
    # else:
    project_url = "https://archive.org/metadata/{}".format(resource_id)
    ...
    # created_seconds = data['created']
    # created_time = datetime.datetime.fromtimestamp(created_seconds).strftime('%Y-%m-%d %H:%M:%S.%f')
    created_time = data['metadata']['addeddate']

    updated_seconds = data['item_last_updated']
    updated_time = datetime.datetime.fromtimestamp(updated_seconds).strftime('%Y-%m-%d %H:%M:%S.%f')
    # updated_time = data['metadata']['modified']
    ...
    resource = {
        ...
        #"date_created": "2019-05-13T14:54:17.129170Z",
        "date_modified": updated_time,
        "date_created": created_time,
        # "date_modified": data['item_last_updated'],
