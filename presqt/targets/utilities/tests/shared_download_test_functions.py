...
def shared_get_success_function_202(test_case_instance):
    ...
    test_case_instance.assertEqual(os.path.isfile('{}/download/{}/data/{}'.format(
...
def shared_get_success_function_202_with_error(test_case_instance):
    ...
    # Verify process_info file status is 'in_progress' initially
    process_info = read_file('mediafiles/jobs/{}/process_info.json'.format(ticket_number), True)
    test_case_instance.assertEqual(process_info['resource_download']['status'], 'in_progress')
    ...
    while process_info['resource_download']['status'] == 'in_progress':
        try:
            process_info = read_file(
                'mediafiles/jobs/{}/process_info.json'.format(ticket_number), True)
        except json.decoder.JSONDecodeError:
            # Pass while the process_info file is being written to
            pass
    ...
    test_case_instance.assertEqual(process_info['resource_download']['status'], 'failed')
    test_case_instance.assertEqual(process_info['resource_download']['message'], test_case_instance.status_message)
    test_case_instance.assertEqual(process_info['resource_download']['status_code'], test_case_instance.status_code)
