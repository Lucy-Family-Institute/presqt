    ...
    for entry in new_file_list:
        if source_target_data and entry['sourceHashes'] == {} or set(
            ...
            if metadata_validation is True:
                return (
                    """{} successful. Fixity can't be determined because {} may not have provided a
                    file checksum. See PRESQT_FTS_METADATA.json for more details.""".format(
                        action, source_target_data['readable_name']))
            else:
                return (
                    """{} successful but with metadata errors. Fixity can't be determined because {}
                    may not have provided a file checksum.""".format(
                        action, source_target_data['readable_name']))
