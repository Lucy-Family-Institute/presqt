...
Target Details
++++++++++++++
'Targets' are endpoints the PresQT API will connect to such as OSF, CurateND, HubZero, etc. Since 
...
structure on the front end. The endpoint is expecting a list of objects with the following structure:
...
------------------------
When hitting the endpoint for details of a specific resource the payload, there is an `extra` field 
which allows for target specific data that might not be universal for all targets to be passed. The 
content of `extra` will change depending on the type of the resource. For instance in OSF the following 
